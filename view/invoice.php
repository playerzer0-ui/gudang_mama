...
<main class="gm-slip-page gm-invoice-page"><div class="gm-invoice-inner">
    <?php if ($hasDocumentSidebar) include __DIR__ . '/invoice_sidebar.php'; ?>
    <section class="document-form">
    <form id="myForm" action="../controller/index.php?action=create_invoice" target="_blank" method="post">
        <header class="gm-slip-heading"><span class="gm-slip-eyebrow">New document</span><div class="gm-slip-title-row"><h1>Invoice</h1><span class="gm-slip-mode"><?php echo htmlspecialchars(str_replace(['amend_invoice_', '_'], ['', ' '], $pageState), ENT_QUOTES, 'UTF-8'); ?></span></div><p>Select a source document, then enter prices and invoice details.</p></header>
...
<section class="gm-slip-section">
            ...
            <?php if($pageState == "moving"){ ?>
                    ...
                    <div class="gm-slip-field"><label for="invoice_date">Tgl invoice *</label><input name="invoice_date" type="date" id="invoice_date" placeholder="di isi" oninput="generateNoInvoice()" required></div>
            <?php } else { ?>
                    <?php if ($pageState == "in") { ?>
                        <div class="gm-slip-field"><label for="no_sj">No SJ *</label><input name="no_sj" type="text" id="no_sj" placeholder="di isi" oninput="getDetailsFromSJ()" required></div>
                        <div class="gm-slip-field"><label for="invoice_date">Tgl invoice *</label><input name="invoice_date" type="date" id="invoice_date" placeholder="di isi" required></div>
                    <?php } else { ?>
                        <div class="gm-slip-field"><label for="no_invoice">No Invoice</label><input name="no_invoice" type="text" id="no_invoice" placeholder="otomatis dari sistem" readonly></div>
            ...
            <?php if($pageState != "moving"){ ?>
                ...
                <?php if ($pageState == "in") { ?>
                    ...
                    <div class="gm-slip-field"><label for="no_invoice">No Invoice *</label><input name="no_invoice" type="text" id="no_invoice" placeholder="di isi" required></div>
                <?php } else { ?>
                    <div class="gm-slip-field"><label for="invoice_date">Tgl invoice *</label><input name="invoice_date" type="date" id="invoice_date" placeholder="di isi" oninput="generateNoInvoice()" required></div>
        ...
        <section id="accountTable" class="gm-slip-section gm-invoice-account"><div class="gm-slip-section-heading"><h2>Invoice total</h2></div><div class="gm-invoice-summary"><div class="gm-slip-field"><label for="no_faktur">No. Faktur *</label><input type="text" name="no_faktur" id="no_faktur" placeholder="di isi" required></div><div class="gm-invoice-totals">
                    ...
                    <div class="gm-invoice-total-row"><label for="tax">PPN (%)</label><input type="number" name="tax" id="tax" value="11" oninput="calculateTotalNominal()"></div>
                    ...
                    <div class="gm-invoice-total-row"><label for="amount_paid">Nilai Dibayar</label><input type="number" inputmode="numeric" name="amount_paid" id="amount_paid" disabled></div>
                </div>
            </div>
        </section>
        <div class="gm-slip-actions"><p>Review prices and tax before saving.</p><div class="gm-invoice-buttons">
        <button type="submit" class="gm-slip-submit" onclick="handleFormSubmit(event)">Create invoice &amp; PDF</button>
    </div>
        </div>
    </form>
    </section>
